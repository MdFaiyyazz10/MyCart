...
export const cartReducer = createReducer({
    cartItem:[],subTotal:0,shippingCharger:0,tax:0,total:0
},{
    ...
    calculatePrice:(state,action)=>{
        ...
        state.subTotal = sum
        state.shippingCharger = state.subTotal > 1000 ? 0 : 200
        state.tax = +(state.subTotal * 0.18).toFixed()
        state.total = state.shippingCharger + state.subTotal + state.tax
    }
